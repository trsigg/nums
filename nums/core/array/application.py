...
class ArrayApplication(object):
    ...
    def _stack_copy(self, X):
        ...
        max_block_rows, max_block_cols = output_block_array.blocks.shape[0], output_block_array.blocks.shape[1]
        ...
        for i in range(max_block_rows):
            ...
            for j in range(max_block_cols):
                ...
                output_block_array.blocks[(i, j)].oid = self.cm.triu_copy(X.blocks[block_row_index].oid, 
                                                                        rows, 
                                                                        cols,
                                                                    syskwargs=syskwargs)
                block_row_index += 1
    ...
    def triu(self, X: BlockArray):
        if len(X.shape) == 1:
            ...
            for block_indices, offset, total_elements in diag_meta:
                syskwargs = {"grid_entry": block_indices, "grid_shape": output_arr_grid.grid_shape}
                output_block_array.blocks[block_indices].oid = self.cm.triu(X.blocks[block_indices].oid, offset, 
                                                                False,
                                                                total_elements,
                                                                syskwargs=syskwargs)
            ...
            for block_indices, offset, total_elements in diag_meta:
                ...
                while row_c < total_row_blocks:
                    syskwargs = {"grid_entry": (row_c, col_c), "grid_shape": output_arr_grid.grid_shape}
                    if (row_c, col_c) in visited:
                        output_block_array.blocks[(row_c, col_c)].oid = self.cm.triu(output_block_array.blocks[(row_c, col_c)].oid, 
                                                                    offset, True, total_elements, syskwargs=syskwargs)
                    else:
                        output_block_array.blocks[(row_c, col_c)].oid = self.cm.triu(X.blocks[(row_c, col_c)].oid, 
                                                                     offset, True, total_elements, syskwargs=syskwargs)
